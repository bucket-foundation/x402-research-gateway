...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| **arXiv (math.*)** | `researched` | `preprint_repository` | Preprints in all math subj classes (AG, AT, CO, DG, NT, PR, ST…) | ~600k math preprints | `http://export.arxiv.org/api/query?search_query=cat:math.*` | none | 1 req/3s polite | arXiv non-exclusive (metadata free; PDF per-paper) | `unknown` | raw (by id), query (search) | `arxiv` | `https://arxiv.org/abs/{id}` | — | Atom XML not JSON; parser new. Rate-limit strict — cache aggressively. Docs: https://info.arxiv.org/help/api/ Endpoints/format: (Atom) |
...
| **Europe PMC** | `researched` | `full_text_repository` | PubMed + preprints + Agricola + patents | ~43M (2026) | `https://www.ebi.ac.uk/europepmc/webservices/rest/search?query=...&format=json` | none | polite | mixed (per-record) | `unknown` | raw, query | `epmc` | `https://europepmc.org/article/{src}/{id}` | — | No API key, simpler than NCBI. Has annotation API for NER. Docs: https://europepmc.org/RestfulWebService |
