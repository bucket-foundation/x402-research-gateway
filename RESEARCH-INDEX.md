...
| **OEIS (Sloane)** | `researched` | `mathematical_object_database` | Integer sequences, formulae, cross-refs | ~380k sequences | `https://oeis.org/search?q={q}&fmt=json` | none | polite | CC-BY-NC 3.0 | `allowed` | raw (`/Axxxxxx`), query (`/search`) | `oeis` | `https://oeis.org/{id}` | — | The canonical combinatorics lookup. Clean JSON. Endpoints/format: record at https://oeis.org/{Axxxxxx} |
...
| **BioModels** | `researched` | `biological_database` | Curated mathematical models of biology | ~3k curated, ~2M auto | `https://www.ebi.ac.uk/biomodels/search?query=...&format=json` | none | polite | CC0 | `allowed` | raw, query | `biomodels` | `https://www.ebi.ac.uk/biomodels/{id}` | — | SBML models, quantitative. |
...
| **OpenAIRE Graph** | `researched` | `scholarly_metadata` | EU-funded OA research graph | ~140M | `https://api.openaire.eu/search/` | none (key optional) | polite | CC-BY | `allowed` | raw, query | `openaire` | `https://explore.openaire.eu/search/publication?pid={doi}` | — | Funding-aware. Endpoints/format: + graph API https://graph.openaire.eu/ |
